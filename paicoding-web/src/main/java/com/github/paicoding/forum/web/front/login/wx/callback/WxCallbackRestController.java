...
public class WxCallbackRestController {
     ...
     * @return
     */
    @PostMapping(path = "/autoRegisterWxUserInfo",
            consumes = {"application/xml", "text/xml"},
            produces = "application/xml;charset=utf-8")
    public BaseWxMsgResVo autoRegisterWxUserInfo(@RequestBody WxTxtMsgReqVo msg) {
        // 执行AI技术圈登录、用户响应问答的场景
        BaseWxMsgResVo res = wxHelper.buildResponseBodyUser(msg.getContent(),msg.getFromUserName());
        fillResVo(res, msg);
        return res;
    }
    /**
    ...
    /**
     * oc使用的和AI技术圈是同一个微信公众号进行授权登录，按照验证码的位数进行区分；我们在这里做一个路由转发
     *
     * @return
     */
