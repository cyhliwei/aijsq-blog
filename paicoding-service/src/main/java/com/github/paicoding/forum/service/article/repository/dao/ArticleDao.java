...
public class ArticleDao extends ServiceImpl<ArticleMapper, ArticleDO> {
    ...
    public ArticleDTO queryArticleByMenuId(String menuId) {
        // 查询文章内容
        LambdaQueryWrapper<ArticleDO> contentQuery = Wrappers.lambdaQuery();
        contentQuery.eq(ArticleDO::getDeleted, YesOrNoEnum.NO.getCode())
                .eq(ArticleDO::getMenuId, menuId)
                .orderByDesc(ArticleDO::getId);
        // 查询文章记录
        List<ArticleDO> lists =  baseMapper.selectList(contentQuery);
        if(!CollectionUtils.isEmpty(lists)){
            ArticleDO article = lists.get(0);
            // 查询文章正文
            ArticleDTO dto = ArticleConverter.toDto(article);
            if (showReviewContent(article)) {
                ArticleDetailDO detail = findLatestDetail(article.getId());
                dto.setContent(detail.getContent());
            } else {
                // 对于审核中的文章，只有作者本人才能看到原文
                dto.setContent("### 文章审核中，请稍后再看");
