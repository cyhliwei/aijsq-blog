...
public class ArticleReadServiceImpl implements ArticleReadService {
    ...
    public ArticleDTO queryDetailArticleByMenuId(String menuId) {
        ArticleDTO article = articleDao.queryArticleByMenuId(menuId);
        if (article == null) {
            throw ExceptionUtil.of(StatusEnum.ARTICLE_NOT_EXISTS, menuId);
        }
        // 更新分类相关信息
        CategoryDTO category = article.getCategory();
        category.setCategory(categoryService.queryCategoryName(category.getCategoryId()));
        ...
        // 更新标签信息
        article.setTags(articleTagDao.queryArticleTagDetails(article.getArticleId()));
        return article;
    }
    /**
    ...
    @Override
    public Boolean queryArticleByMenuId(String menuId) {
        ArticleDTO article = articleDao.queryArticleByMenuId(menuId);
        if (article == null) {
            return false;
        }
    ...
    @Override
    public ArticleDTO queryArticleListByMenuId(String menuId, Long readUser) {
        ArticleDTO article = queryDetailArticleByMenuId(menuId);

        // 文章阅读计数+1
        countService.incrArticleReadCount(article.getAuthor(), article.getArticleId());

        // 文章的操作标记
        if (readUser != null) {
            // 更新用于足迹，并判断是否点赞、评论、收藏
            UserFootDO foot = userFootService.saveOrUpdateUserFoot(DocumentTypeEnum.ARTICLE, article.getArticleId(),
                    article.getAuthor(), readUser, OperateTypeEnum.READ);
            article.setPraised(Objects.equals(foot.getPraiseStat(), PraiseStatEnum.PRAISE.getCode()));
            article.setCommented(Objects.equals(foot.getCommentStat(), CommentStatEnum.COMMENT.getCode()));
            article.setCollected(Objects.equals(foot.getCollectionStat(), CollectionStatEnum.COLLECTION.getCode()));
        ...
        // 更新文章统计计数
        article.setCount(countService.queryArticleStatisticInfo(article.getArticleId()));

        // 设置文章的点赞列表
        article.setPraisedUsers(userFootService.queryArticlePraisedUsers(article.getArticleId()));
        return article;
    }
}
